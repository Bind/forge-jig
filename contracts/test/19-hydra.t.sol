...
import {HydraJig} from "contracts/jig/HydraJig.sol";

import "contracts/19-hydra.sol";
...
contract HydraTest is DSTest {
    ...
    function testHydra() public {
        ...
        uint8[] memory fetchedMoves = b.getMoveset(address(0), 0, 0);
        assert(fetchedMoves[0] == 20);
        assert(fetchedMoves[1] == 21);
        uint256[] memory fetchedActions = b.getActionLog(address(0), 0, 0);
        assert(fetchedActions[0] == 300);
        assert(fetchedActions[1] == 301);
    }

    function testMovest() public {
        uint256 t = 100;
        uint8 h = 12;
        uint8 d = 14;
    ...
    function testRandPostion(
        ...
        jig.entities(owner, id, pos, villain);
        uint8[] memory fetchedMoves = b.getMoveset(owner, id, pos);
        assert(fetchedMoves[0] == 20);
        assert(fetchedMoves[1] == 21);
        uint256[] memory fetchedActions = b.getActionLog(owner, id, pos);
        assert(fetchedActions[0] == 300);
        assert(fetchedActions[1] == 301);
    }
}
